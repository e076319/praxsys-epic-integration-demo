/* PraxSys Epic Integration Demo V1.3.2 - Epic Sandbox SMART on FHIR bootstrap */
(function(){
  'use strict';
  const CONFIG = {
  ...
  function ldaEntries(ctx){
    return (ctx && ctx.lda && Array.isArray(ctx.lda.entry)) ? ctx.lda.entry : [];
  }
  function ldaCodeLabel(resource){
    const c=resource && resource.code;
    if(!c) return 'Not returned';
    if(c.text) return c.text;
    const coding=(c.coding||[])[0]||{};
    return coding.display || coding.code || 'Not returned';
  }
  function ldaValue(resource){
    if(!resource) return 'Not returned';
    const keys=['valueString','valueDateTime','valueDate','valueInteger','valueDecimal','valueBoolean'];
    for(const k of keys){ if(resource[k] !== undefined && resource[k] !== null) return String(resource[k]); }
    if(resource.valueCodeableConcept){
      const cc=resource.valueCodeableConcept;
      const coding=(cc.coding||[])[0]||{};
      return cc.text || coding.display || coding.code || 'Codeable value returned';
    }
    if(resource.valueQuantity){
      return [resource.valueQuantity.value,resource.valueQuantity.unit].filter(v=>v!==undefined&&v!==null&&v!=='').join(' ') || 'Quantity returned';
    }
    return 'See FHIR JSON';
  }
  function showLdaData(ctx){
    ctx=ctx || window.PRAXSYS_EPIC_CONTEXT || {};
    const em=$('integrationModal'); if(!em) return;
    const dlg=em.querySelector('.integration-dialog');
    const entries=ldaEntries(ctx);
    const first=entries[0] && entries[0].resource ? entries[0].resource : null;
    const when=first && (first.effectiveDateTime || first.issued || (first.effectivePeriod && first.effectivePeriod.start));
    dlg.innerHTML=`<button class="integration-close" id="integrationClose" aria-label="Close">×</button>
      <div class="section-label">Epic Sandbox LDA Data · Live</div><h2>${entries.length} LDA Observation${entries.length===1?'':'s'} returned</h2>
      <p>The information below is taken from the actual Epic sandbox FHIR response for the patient selected during this SMART session.</p>
      ${first?`<div class="integration-status-grid">
        <div><strong>Observation</strong><span>${escapeHtml(ldaCodeLabel(first))}</span></div>
        <div><strong>Value</strong><span>${escapeHtml(ldaValue(first))}</span></div>
        <div><strong>FHIR Observation ID</strong><span>${escapeHtml(text(first.id))}</span></div>
        <div><strong>Effective / issued</strong><span>${escapeHtml(text(when))}</span></div>
        <div><strong>Status</strong><span>${escapeHtml(text(first.status))}</span></div>
        <div><strong>Patient context</strong><span>${escapeHtml(text(ctx.patientId))}</span></div>
      </div>`:'<div class="epic-data-note">No LDA Observation entries were returned in this session.</div>'}
      <details class="disclosure" open><summary>View complete Epic LDA FHIR JSON</summary><pre class="epic-json">${escapeHtml(JSON.stringify(ctx.rawLda || ctx.lda || {},null,2))}</pre></details>
      ${ctx.ldaError?`<div class="epic-data-note"><strong>LDA API error:</strong> ${escapeHtml(ctx.ldaError)}</div>`:''}`;
    em.hidden=false;
  ...
  function showIntegrationModal(ctx){
    ...
    dlg.innerHTML=`<button class="integration-close" id="integrationClose" aria-label="Close">×</button>
      ...
      ${ldaEntries(ctx).length?'<div class="button-row"><button class="button" id="viewEpicLdaFromStatus" type="button">View Epic LDA Data</button></div>':''}
    ...
    em.hidden=false;
    $('integrationClose').onclick=()=>em.hidden=true;
    const ldaBtn=$('viewEpicLdaFromStatus'); if(ldaBtn) ldaBtn.onclick=()=>showLdaData(ctx);
  }
  ...
  window.PraxSysEpicIntegration={
    showLdaData:()=>showLdaData(window.PRAXSYS_EPIC_CONTEXT||{}),
    showIntegrationStatus:()=>showIntegrationModal(window.PRAXSYS_EPIC_CONTEXT||{})
  };
