...
(function () {
  ...
  function renderPatients(){const tiles=Object.values(window.PRAXSYS_PATIENTS).map(p=>`<button class="tile" data-patient="${p.id}"><h3>${p.name}</h3><p>${p.mrn} · ${p.age} years · ${p.unit} ${p.room}</p><span class="source-pill">Epic SMART context · simulated</span><span class="source-pill">Active central line</span></button>`).join('');return `<div class="card"><div class="section-label">PraxSys Epic Integration Demo</div><h1 class="card-title">Select a patient for CLABSI Prevention</h1><div class="card-subtitle">Demonstration Medical Center · Clinician/Observer workflow. Patient, encounter, and location context are modeled on Epic SMART on FHIR launch context. Synthetic data only.</div></div><div class="tile-grid">${tiles}</div>`;}
  ...
  function renderClabsiPreventionHome(){const p=patient();return shell(`<button class="back-link" data-back="patients">← Back</button><div class="card"><div class="section-label">CLABSI Prevention Pathway</div><h1 class="card-title">Central line prevention and maintenance observation</h1><div class="card-subtitle">Focused Epic integration demonstration using the existing CLABSI Prevention pathway.</div><div class="button-row"><button class="button primary" id="startClabsiPreventionPathway">Begin prevention review</button></div></div><div class="ehr-panel" style="margin-top:14px"><div class="section-label">Epic-sourced context · simulated</div><div class="ehr-grid"><div class="data-cell"><div class="data-label">Active LDA</div><div class="data-value">${p.centralLineType}</div><div class="mapping-tag verified">Verified API pattern · Epic FHIR Observation (LDA)</div></div><div class="data-cell"><div class="data-label">Insertion site</div><div class="data-value">${p.centralLineSite}</div><div class="mapping-tag verified">Verified API pattern · LDA property</div></div><div class="data-cell"><div class="data-label">Insertion date/time</div><div class="data-value">${p.centralLineInserted}</div><div class="mapping-tag verified">Verified API pattern · LDA property</div></div><div class="data-cell"><div class="data-label">Documented indication</div><div class="data-value">${p.centralLineIndication}</div><div class="mapping-tag configured">Hospital-configured mapping</div></div></div></div>`,`<div class="card"><div class="section-label">Integration legend</div><div class="integration-legend"><span class="mapping-tag verified">Epic-documented API pattern</span><span class="mapping-tag configured">Hospital-configured mapping</span><span class="mapping-tag observed">PraxSys observation</span></div><div class="card-subtitle" style="margin-top:12px">The demo does not claim a live Epic connection. It distinguishes documented Epic capabilities from mappings that an individual hospital must configure.</div></div>`);}
  function renderClabsiPreventionObservation(){const d=clabsiPreventionStep();let body='';let can=false;
    if(d.step===1){body=`<div class="ehr-panel"><div class="section-label">Information from Epic · simulated</div><div class="data-cell"><div class="data-label">Current central line order / indication</div><div class="data-value">${patient().centralLineIndication}</div><div class="mapping-tag configured">Hospital-configured Epic mapping · exact source varies by organization</div></div></div><label class="ack-row"><input type="checkbox" id="clabsiPreventionAck" ${state.clabsiPreventionAck?'checked':''}><span>I acknowledge that I reviewed the current central line order and documented indication.</span></label>`;can=state.clabsiPreventionAck;}
