/* PraxSys Epic Integration Demo V1.3 - Epic Sandbox SMART on FHIR bootstrap */
(function(){
  'use strict';
  const CONFIG = {
  ...
  function showIntegrationModal(ctx){
    ...
    dlg.innerHTML=`<button class="integration-close" id="integrationClose" aria-label="Close">×</button>
      ...
      <details class="disclosure"><summary>View Epic LDA FHIR JSON</summary><pre class="epic-json">${escapeHtml(JSON.stringify(ctx.rawLda||{},null,2))}</pre></details>
      <details class="disclosure"><summary>View SMART token context</summary><pre class="epic-json">${escapeHtml(JSON.stringify(ctx.tokenResponse||{},null,2))}</pre></details>
  ...
  window.PraxSysEpicIntegration={
    showIntegration:()=>window.PRAXSYS_EPIC_CONTEXT && showIntegrationModal(window.PRAXSYS_EPIC_CONTEXT),
    showLdaData:()=>{
      const ctx=window.PRAXSYS_EPIC_CONTEXT||{};
      showIntegrationModal(ctx);
