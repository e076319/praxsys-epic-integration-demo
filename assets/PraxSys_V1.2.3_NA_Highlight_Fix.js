...
(function () {
  ...
  function mapEpicLda(obs){
    ...
    return {
      ...
      line_status:firstText(obs.status,'active'),
      line_active:String(obs.status||'').toLowerCase()!=='inactive'&&String(obs.status||'').toLowerCase()!=='entered-in-error',
      latest_lda_assessment:assessment,
  ...
  function renderClabsiPreventionHome(){
    ...
    const livePanel=m
      ? `<div class="ehr-panel" style="margin-top:14px"><div class="section-label">Epic Live · LDA mapping</div><div class="card-subtitle" style="margin-bottom:12px">Values in this panel were returned by the current Epic sandbox FHIR session and mapped to PraxSys canonical fields. Original Epic text is preserved where available; unfamiliar values are not guessed.</div><div class="ehr-grid">${epicMappingCell('Line type',m.line_type,'line_type',m.line_type_source_text)}${epicMappingCell('Line status',m.line_status,'line_status',m.line_status)}${epicMappingCell('Insertion site',m.line_body_site,'line_body_site',m.line_body_site_source)}${epicMappingCell('Insertion date/time',m.line_insertion_datetime,'line_insertion_datetime','')}${epicMappingCell('Insertion method',m.line_insertion_method,'line_insertion_method',m.line_insertion_method_source)}${epicMappingCell('Latest LDA assessment',m.latest_lda_assessment,'latest_lda_assessment','')}</div><div class="card-subtitle" style="margin-top:12px">FHIR Observation ID: ${displayOrNotAvailable(m.line_id)}${live.mapped.length>1?` · ${live.mapped.length} actual LDA Observations returned`:''}</div></div>`
      : `<div class="ehr-panel" style="margin-top:14px"><div class="section-label">Epic Live · LDA mapping</div><div class="card-subtitle"><strong>Epic connection succeeded.</strong> No actual FHIR Observation resource was returned for this patient's LDA search. PraxSys will not invent, infer, or relabel demonstration line values as Epic Live.</div></div>`;
    const demoPanel=`<div class="ehr-panel" style="margin-top:14px"><div class="section-label">Demonstration Clinical Scenario</div><div class="card-subtitle" style="margin-bottom:12px">Hospital-configured sample data · not returned by Epic.</div><div class="ehr-grid"><div class="data-cell"><div class="data-label">Scenario line type</div><div class="data-value">${p.centralLineType}</div><div class="mapping-tag configured">Demonstration value · not Epic Live</div></div><div class="data-cell"><div class="data-label">Scenario insertion site</div><div class="data-value">${p.centralLineSite}</div><div class="mapping-tag configured">Demonstration value · not Epic Live</div></div><div class="data-cell"><div class="data-label">Scenario insertion date/time</div><div class="data-value">${p.centralLineInserted}</div><div class="mapping-tag configured">Demonstration value · not Epic Live</div></div><div class="data-cell"><div class="data-label">Documented indication</div><div class="data-value">${p.centralLineIndication}</div><div class="mapping-tag configured">Clinician-confirmed / hospital mapping for demo</div></div></div></div>`;
  ...
  function bind(){
    ...
    const cw=document.getElementById('changeWorkspace');if(cw)cw.onclick=()=>{state.view='role';render();};
